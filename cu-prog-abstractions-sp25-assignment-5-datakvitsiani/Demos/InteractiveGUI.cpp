/* Interactive environment for testing a hash table. */
...
namespace {
    const string kButtonFont = Font(FontFamily::MONOSPACE, FontStyle::NORMAL, 12, Color::BLACK()).stanfordCPPLibFontString();
    ...
    InteractiveGUI<Table>::InteractiveGUI(GWindow& window, const string& tableName) : ProblemHandler(window) {
        ...
        console->doWithStyle(Color::fromHex(0x008000), [&] {
    ...
    void logEffectsOf(GColorConsole& g, Function fn, Box<T>) {
        auto value = fn();
        g.doWithStyle(Color::fromHex(0x008000), [&] {
    ...
    bool InteractiveGUI<Table>::performSafely(const string& command, Function fn) {
        *console << stringReplace(command, "%s", "table" + to_string(index));
        try {
            ...
            console->doWithStyle(Color::fromHex(0x800000), [&, this] {
                *console << " // Triggered a call to error(\"" << e.what() << "\");" << endl;
            });
            return false;
        } catch (const exception& e) {
            console->doWithStyle(Color::fromHex(0xFF0000), GColorConsole::BOLD_ITALIC, [&, this] {
